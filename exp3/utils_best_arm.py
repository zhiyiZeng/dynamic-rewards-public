...
def calc_maxdrawdown(portfolio):
    ...
    return mdd, mdd_start_index, mdd_end_index
...
def plot_return(mode, DATASET_NAME, data_loader, train_portfolios, test_portfolios, colors, indexes):
    ...
    for k, color in zip(items, colors):
        ...
        mdd1, mdd_start_index1, mdd_end_index1, _, _ = calc_maxdrawdown(df["portfolio"])
        d = indexes[k]
...
def calc_all(mode, dt, portfo_percentage, bh_percentage):
    # FIXME: 这里资产净值算的对吧
    portfo = (1 + portfo_percentage).cumprod()
    dt[f"cumreturn_{mode}"] = (portfo - 1).iloc[-1]
    # NOTE 乘以np.sqrt(252)是年化
    dt[f"mdd_{mode}"], _, _ = calc_maxdrawdown(portfo)
...
def calc_return(bh_percentage, test_portfolios,):
    """
    这里portfo输入就是pct_change
    """
    res = []
    
    for key in test_portfolios.keys():
        dt = {}
        # FIXME: bh是有问题的，这里应该输入整个一段，而不是单独一段
        portfo_percentage = pd.Series(test_portfolios[key])
        # TODO: 我估计这个length有问题。
        ...
        calc_all("test", dt, portfo_percentage, bh_percentage)

        dt = dict(sorted(dt.items(), key=lambda x: x[0], reverse=True))
        res.append(dt)

    res = pd.DataFrame(res, index=list(test_portfolios.keys()))
    res = res.fillna(0)
    res = res.T
    return res
