    ...
    def arithmetic_return(self):
        ...
        for i in range(len(self.data)):
            if (self.data[self.action_label][i] == 'buy') or (own_share and self.data[self.action_label][i] == 'None'):
                own_share = True
                if i < len(self.data) - 1:
                    # NOTE 这里不用加交易费用，因为self.get_daily_portfolio_value()已经算了交易费用，那个函数算了share，所以才是实际的。
                    self.data[f'arithmetic_daily_return_{self.action_label}'][i] = \
                        (self.data['close'][i + 1] - self.data['close'][i]) / self.data['close'][i]

            # NOTE 因为profit是用i+1 - i来算的，所以sell的时候就不用再算了。
            elif self.data[self.action_label][i] == 'sell':
                own_share = False

        self.data[f'arithmetic_daily_return_{self.action_label}'] = self.data[f'arithmetic_daily_return_{self.action_label}'] * 100
        # else you have sold your share, so you would not lose or earn any more money
        return self.data[f'arithmetic_daily_return_{self.action_label}']
    ...
    def get_daily_portfolio_value(self):
        ...
        for i in range(len(self.data)):
            ...
            if action == 'buy' and num_shares == 0:  # then buy and pay the transaction cost
                num_shares = portfolio_value[-1] * (1 - self.trading_cost_ratio) / self.data.iloc[i]['close']
