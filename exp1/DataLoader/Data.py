...
class Data:
    ...
    def get_reward(self, action, lamb, reward_type):
        ...
        if reward_type == "future_profit_1":
            p1 = self.close_price[reward_index_first]
            index_last = self.current_state_index + self.start_index_reward + 1 \
                            if self.current_state_index + 1 < len(self.states) else len(self.close_price) - 1
            p2 = self.close_price[index_last]
            if action == 0 or (action == 1 and self.own_share):  # Buy Share or Hold Share
                reward = (1 - self.trading_cost_ratio) * p2 / p1 - 1  # profit in percent
            elif action == 2 or (action == 1 and not self.own_share):  # Sell Share or No Share
                reward = (1 - self.trading_cost_ratio) * p1 / p2 - 1
        ...
        if reward_type == "future_profit_10":
            p1 = self.close_price[reward_index_first]
            index_last = self.current_state_index + self.start_index_reward + 10 \
        ...
        if reward_type == "future_profit_15":
            p1 = self.close_price[reward_index_first]
            index_last = self.current_state_index + self.start_index_reward + 15 \
                            if self.current_state_index + 15 < len(self.states) else len(self.close_price) - 1
            p2 = self.close_price[index_last]
        ...
        if reward_type == "future_profit_20":
            p1 = self.close_price[reward_index_first]
            index_last = self.current_state_index + self.start_index_reward + 20 \
                            if self.current_state_index + 20 < len(self.states) else len(self.close_price) - 1
            p2 = self.close_price[index_last]
        ...
        if reward_type == "old_sharpe":
            reward_index_first = self.start_index_reward
            reward_index_last = self.current_state_index 
            reward = self.calc_sharpe(reward_index_first, reward_index_last)
